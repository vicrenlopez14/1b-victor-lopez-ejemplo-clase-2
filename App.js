...
const App = () => {
    ...
    const agregarCliente = () => {
        ...
        // const nuevoCliente = {id: clientes.length + 1, nombre: nombre, cantPersonas: cantPersonas, fechaReserva: fechaReserva};
        // Agrega el nuevo cliente a la lista de clientes
        const remapId = clientes.map((cliente, index) => {
            return {
                ...cliente,
                id: index + 1
            }
    ...
    const eliminarCliente = (id) => {
        ...
        const deletedExcluded = clientes.filter((cliente) => cliente.id !== id)
        // Actualiza la lista de clientes
        // Remapeando los ID de los clientes para que sean consecutivos
        const remapId = deletedExcluded.map((cliente, index) => {
            return {
                ...cliente,
                id: index + 1
            }
            ...
            >
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <TouchableOpacity onPress={showDatepicker}><Text>Seleccionar fecha de
                            Reserva</Text></TouchableOpacity>
            ...
            <FlatList
                ...
                renderItem={({item}) => (
                    ...
                    >
                        ...
                        <Text style={styles.clienteNombre}>Personas: {item.cantPersonas}</Text>
                        <Text style={styles.clienteFecha}>
                            Fecha de Reserva: {item.fechaReserva.toDateString()}
                        </Text>
                        <Button color="red" title="Eliminar" onPress={() => eliminarCliente(item.id)}
                        />
                    </TouchableOpacity>
                )}
